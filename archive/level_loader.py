...
from modules.settings import (
    ...
    LEVEL_THRESHOLD, PIPE_SPAWN_RATE_FRAMES, PIPE_VARIANT_COLORS, COLORS
)
...
def default_level_config():
    ...
    return {
        ...
        'pipe_spawn_rate_frames': PIPE_SPAWN_RATE_FRAMES,
        'num_obstacles': 3,
...
def get_level_background(level_config):
    ...
    """
    return level_config.get('background', 'background_default.png')
...
def add_obstacle(level_config, obstacles, screen_width):
    ...
    obstacles.append(pipe)
    logging.info(f"New obstacle added at x={screen_width} with color={pipe.color}.")
...
def handle_level_progression(score, current_level, background, screen, obstacles, level_config):
    ...
    if score != 0 and score % LEVEL_THRESHOLD == 0:
        ...
        PIPE_SPAWN_RATE_FRAMES = max(60, new_level_config.get('pipe_spawn_rate_frames', PIPE_SPAWN_RATE_FRAMES) - 10)
