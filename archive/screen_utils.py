...
def start_screen(screen, font):
    ...
    title_text = font.render("Starbird", True, settings.COLORS['BLUE'])
    instruction_text = font.render("Press any key to start", True, settings.COLORS['WHITE'])
...
def game_over_screen(screen, font, score, high_scores):
    ...
    screen.fill(settings.COLORS['GAME_OVER'])
    game_over_text = font.render("Game Over", True, settings.COLORS['RED'])
    score_text = font.render(f"Your Score: {score}", True, settings.COLORS['WHITE'])
    high_score_text = font.render(f"High Score: {high_scores['top_score']} by {high_scores['player']}", True, settings.COLORS['WHITE'])
    retry_text = font.render("Press R to Retry or Q to Quit", True, settings.COLORS['WHITE'])
...
def pause_game(screen, font):
    ...
    paused = True
    pause_text = font.render("Paused. Press P to Resume.", True, settings.COLORS['WHITE'])
