...
class ScrollingBackground:
    ...
    def load_primary_background(self):
        ...
        try:
            background_path = os.path.join('assets', 'backgrounds', BACKGROUND_IMAGE)
            ...
            self.x2 = self.width
            self.y = 0
            logging.info(f"Primary background image loaded successfully from {background_path}.")
        except FileNotFoundError as e:
            logging.error(f"Background image not found: {e}")
            # Create a solid color background as a fallback
    ...
    def load_background_layers(self):
        ...
        try:
            for i, layer_filename in enumerate(BACKGROUND_LAYERS):
                layer_path = os.path.join('assets', 'backgrounds', layer_filename)
    ...
    def initialize_stars(self):
        ...
        self.stars = [(random.randint(0, WIDTH), random.randint(0, HEIGHT)) for _ in range(STAR_COUNT)]
        self.star_color = STAR_COLOR
        self.star_speed = STAR_SPEED

    def initialize_comet(self):
        """Initializes comet effect settings."""
        self.comet_position = [-COMET_SIZE, random.randint(0, HEIGHT)]
        self.comet_speed_x = COMET_SPEED[0]
        self.comet_speed_y = COMET_SPEED[1]
    ...
    def update_comet(self):
        ...
        self.comet_position[0] += self.comet_speed_x
        self.comet_position[1] += self.comet_speed_y
        if self.comet_position[0] > WIDTH or self.comet_position[1] > HEIGHT:
            self.comet_position = [-COMET_SIZE, random.randint(0, HEIGHT)]
    ...
    def draw_comet(self, screen):
        """Draws the comet."""
        pygame.draw.circle(screen, self.comet_color, (int(self.comet_position[0]), int(self.comet_position[1])), COMET_SIZE)

    def draw_black_hole(self, screen):
        """Draws a black hole at the center of the screen."""
        pygame.draw.circle(screen, BLACK_HOLE_COLOR, (WIDTH // 2, HEIGHT // 2), BLACK_HOLE_RADIUS)
    ...
    def load_new_background(self, layer_filename):
        ...
        try:
            ...
            logging.info(f"New background layer loaded from {layer_path}.")
        except pygame.error as e:
