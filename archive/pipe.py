...
class Pipe:
    ...
    """
    def __init__(self, x, speed=PIPE_SPEED, pipe_gap=PIPE_GAP, color=None):
        """
        ...
        Args:
            ...
            color (tuple, optional): RGB color tuple for the pipe. If None, a random color is chosen.
        """
        self.speed = speed
        ...
        self.color = color if color else random.choice(PIPE_VARIANT_COLORS)
    ...
    def increase_speed(self, level):
        ...
        logging.info(f"Pipe speed increased to {self.speed} based on level {level}.")
    ...
    def is_passed(self, bird):
        ...
        if not self.passed and bird.rect.left > self.top_rect.right:
            self.passed = True
            logging.info("Pipe passed by the bird.")
            return True
        return False

def add_pipe(obstacles, x_position, color=None):
    """
    ...
    Args:
        ...
        color (tuple, optional): RGB color tuple for the pipe. If None, a random color is chosen.
    """
    pipe = Pipe(x_position, color=color)
    obstacles.append(pipe)
    logging.info(f"New pipe added at x={x_position} with color={pipe.color}.")
...
def remove_off_screen_pipes(obstacles):
    ...
    after_count = len(obstacles)
    removed = before_count - after_count
    if removed > 0:
        logging.info(f"Removed {removed} off-screen pipes.")
...
def check_collision(bird, pipes):
    ...
    for pipe in pipes:
        if bird.rect.colliderect(pipe.top_rect) or bird.rect.colliderect(pipe.bottom_rect):
            logging.info("Collision detected between bird and pipe.")
            return True
    return False
